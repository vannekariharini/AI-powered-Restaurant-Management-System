...
#importing libraries for ai functionality
import warnings
...
with open('savedModels/updated_decision_tree_model.pkl', 'rb') as file:
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if account:
            ...
            query2 = 'INSERT INTO {} (usedname, email, password, dob, mobile) VALUES ( %s, %s, %s, %s,%s)'.format(selloruser)
...
#==================================================== SELLER - SALES PREDICTION =============================================================================
def seller_predict(request):
            cursor = mydb.cursor()
            #using openweather api to get today's temperature in hyderabad
            BASE_URL = "http://api.openweathermap.org/data/2.5/weather?"
            API_KEY = "5701650be23fc7ecc4ed1df46e01dc02"
            CITY ="Hyderabad,IN"
            url= BASE_URL + "appid=" + API_KEY + "&q=" + CITY
            response = requests.get(url).json()
            todays_temperature= round(response['main']['temp']-273.15)
            day_of_the_week=dt.datetime.today().weekday()
            days_of_week=['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
            todays_humidity=response['main']['humidity']
    ...
    'very heavy rain'
            ...
            encoded_weather_description=weather_descriptions.index(current_weather_description)
            todays_sales_prediction= decision_tree_model.predict([[ day_of_the_week,todays_temperature,todays_humidity,encoded_weather_description]] )
            todays_temperature_in_str=str(todays_temperature)+'°C'

            # Get the current date and time in India Standard Time (IST)
            ist = pytz.timezone('Asia/Kolkata')
            ist_now = dt.datetime.now(ist)

            # Print the current date in IST
            date=str(ist_now.strftime("%d-%m-%Y"))
            
            cursor.close()
            return render(request, 'seller_predict.html', {'todays_sales_prediction': todays_sales_prediction[0].upper(),'todays_temperature':todays_temperature_in_str,'todays_date':date,'day_of_the_week':days_of_week[day_of_the_week],'todays_humidity':todays_humidity,'weather_description':current_weather_description})